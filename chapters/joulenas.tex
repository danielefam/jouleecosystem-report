...
\end{itemize}
While this chapter explains the core optimization mechanisms and campaign commands, the repository documents provide exact configuration details. Unlike JouleGrad, which is public on GitHub, the JouleNAS codebase is currently private; its mathematical formulation, hyperparameter schedules, and empirical Pareto curves are fully documented here to ensure complete reproducibility.

\section{How the mask optimization works}

During mask optimization, only the mask logits are updated; backbone network weights and BatchNorm statistics remain frozen. For each channel $i$, the gating mechanism relies on five variables:

\begin{itemize}
    \item $m_i \in (-\infty, +\infty)$ (\textbf{mask logit}): A learnable score for channel $i$, updated by gradient descent. A positive score retains the channel; a negative score prunes it.
    \item $\tau \in (0, +\infty)$ (\textbf{temperature}): An annealing parameter that sharpens the sigmoid curve over time. It initializes at $1.0$ and halves every 10 epochs ($1.0 \to 0.5 \to 0.25 \dots$).
    ...
    $$
    When $m_i = 0$, $p_i = 0.50$.
    \item $\theta \in [0, 1]$ (\textbf{threshold}): The decision cutoff. We set $\theta = 0.49$ for the first 20 epochs, then tighten it to $0.50$.
    \item $g_i \in \{0, 1\}$ (\textbf{binary gate}): The discrete execution switch for channel $i$, computed with the Heaviside step function $H$:
    $$
    g_i = H(p_i - \theta) = \begin{cases} 1 & \text{if } p_i \ge \theta \\ 0 & \text{if } p_i < \theta \end{cases}
    $$
\end{itemize}

During the forward pass, channel $i$'s feature map is multiplied by its gate $g_i$. If $g_i = 1$, features pass unmodified; if $g_i = 0$, the channel is zeroed out. In the backward pass, gradients flow through the step function back to the logits $m_i$ via the straight-through estimator (STE).

As training proceeds and the temperature $\tau$ decays, the soft probabilities $p_i$ polarize toward 0 or 1.

\section{How the energy term is computed}

The regularization term $\widehat{E}$ estimates inference energy using JouleGrad. Below, we define effective layer dimensions and explain how gradients backpropagate through the lookup grid.

\subsection{Discrete versus continuous energy modes}
\label{subsec:energy_mode}
JouleNAS provides two formulations for computing effective layer sizes from learned masks:
...
\begin{enumerate}
    ...
    $$
    where $C_\ell$ is the total channel capacity of layer $\ell$.
    This looks up pruned model energy using only active channels.
    During backpropagation, gradients pass through the straight-through estimator: $\partial g_i / \partial m_i \approx p_i (1 - p_i) / \tau$.

    \item \textbf{Continuous mode} (\texttt{--energy-mode continuous}):
    The effective output width $d^{(\ell)}_{\text{eff}}$ is the sum of soft channel probabilities $p^{(\ell)}_i$:
    $$
    d^{(\ell)}_{\text{eff}} = \sum_{i=1}^{C_\ell} p^{(\ell)}_i = \sum_{i=1}^{C_\ell} \sigma(m^{(\ell)}_i / \tau).
    $$
    Here $d^{(\ell)}_{\text{eff}}$ spans continuous values in $[0, C_\ell]$, representing the expected number of active channels.
    Because JouleGrad employs multilinear interpolation, it evaluates fractional dimensions smoothly to return $\widehat{E}$.
    In this mode, the step function $H$ is bypassed in the energy path, eliminating step discontinuities and yielding an exact analytical gradient for the energy penalty:
    $\partial \widehat{E} / \partial m_i = (\partial \widehat{E} / \partial d_{\text{eff}}) \cdot (p_i (1 - p_i) / \tau)$.
    (While task evaluation still applies binary gates $g_i$ via STE to evaluate true pruned accuracy, the energy penalty branch requires no surrogate approximation).
\end{enumerate}
...
Each run outputs a metrics CSV (tracking accuracy, active channels, and energy) and a final mask checkpoint, ready for physical deployment (tested on ResNet-18 in Chapter~\ref{chap:bridge} and discussed as future work in Chapter~\ref{chap:future_work}).

\section{Experimental results and what they tell us}
\label{sec:experimental_results}

We evaluated JouleNAS by applying energy-constrained mask optimization to ResNet-18 across two benchmark computer vision datasets: CIFAR-10 ($32 \times 32$ image resolution) and Imagenette ($224 \times 224$ image resolution). We ran experiments on two distinct edge platforms with very different architectures and energy footprints:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Non-monotonic behavior}: On CIFAR-10 in discrete mode, the network collapses at a lower penalty ($\lambda_E = 0.05$), but succeeds at a harsher penalty ($\lambda_E = 0.10$).
...
\subsection{Amortizing GPU execution overhead: ResNet-18 pruning at batch size 32}
\label{sec:agxorin_bs32_experiments}

\noindent\textit{[Note: This subsection was written by the AI assistant to document and analyze the batch-32 scaling experiments and Pareto optimization results on NVIDIA Jetson AGX Orin. The content has not been revised.]}
\vspace{0.5em}

Throughout the previous sections, all measurements and pruning searches operated under a single-inference constraint ($\text{batch size} = 1$). While batch~1 reflects hard real-time edge robotics scenarios where inputs arrive sequentially without latency tolerance, modern embedded accelerators such as the NVIDIA Jetson AGX Orin possess massive parallel compute capabilities (2048 Ampere CUDA cores) that remain severely underutilized when processing individual $32 \times 32$ images.

\subsubsection{Theoretical motivation: static launch latency vs. compute-bound pruning}

On GPUs, executing a neural network layer incurs non-negligible fixed runtime overheads: CUDA kernel launches, driver queue dispatching, context switching, memory synchronizations, and idle baseline board power. At $\text{batch size} = 1$, these static overheads dominate total energy: the dense CIFAR-10 ResNet-18 consumes $18.51\text{ mJ}$ per inference. In this latency-dominated regime, pruning convolutional channels reduces theoretical arithmetic FLOPs, but the GPU execution time does not decrease proportionally because the kernel launch latency remains constant.

Increasing the batch size to $32$ fundamentally alters this hardware regime:
\begin{enumerate}
    \item \textbf{Amortization of kernel launches}: The fixed invocation cost of each CUDA kernel is amortized across 32 parallel images.
    \item \textbf{Streaming multiprocessor (SM) saturation}: The larger workload fills the GPU warps, transitioning the device from a memory latency / launch-overhead bounded regime into a compute-dense regime where execution time and dynamic power scale directly with active MACs.
    \item \textbf{Energy reduction}: As measured in the dedicated AGX Orin batch-32 lookup table, the dense inference energy drops by a factor of $13.5\times$, from $18.51\text{ mJ}$ to only $1.372\text{ mJ}$ per sample (total batch energy: $43.9\text{ mJ}$).
\end{enumerate}

\subsubsection{Validating the inverse energy scaling law across batch sizes}

Recall the scaling relationship formalized in Equation~\ref{eq:cross_device_lambda_scaling}. Because the empirical energy per inference drops by $13.49\times$ at batch size 32 ($1.372\text{ mJ}$ vs. $18.51\text{ mJ}$), the energy penalty $\lambda_E \widehat{E}$ in the objective function produces an energy gradient that is over thirteen times smaller. Consequently, applying the same penalty weight that succeeded at batch~1 ($\lambda_E = 0.005$) results in negligible pruning pressure.

According to Equation~\ref{eq:cross_device_lambda_scaling}, the penalty weight must be scaled upwards:
\begin{equation}
\lambda_{E, \text{BS32}} \approx \lambda_{E, \text{BS1}} \cdot \frac{E_{\text{BS1}}}{E_{\text{BS32}}} \approx \lambda_{E, \text{BS1}} \cdot 13.5.
\end{equation}
To systematically explore this regime, we conducted a campaign across 7 penalty weights $\lambda_E \in [0.0, 2.50]$ in discrete mode on CIFAR-10. Table~\ref{tab:joulenas_results_bs32} details the final performance, structural channel sparsity, and energy metrics across all runs, and Figure~\ref{fig:pareto_frontier_bs32} illustrates the empirical Pareto frontier and sparsity trade-offs.

\begin{table}[htbp]
\centering
\small
\caption{Summary of JouleNAS pruning experiments on CIFAR-10 evaluated with the NVIDIA Jetson AGX Orin batch size 32 lookup table (Dense baseline energy: $1.372\text{ mJ}$ per sample; baseline test accuracy: $91.86\%$). All runs executed in discrete mode with $100$ epochs.}
\label{tab:joulenas_results_bs32}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lcccccc}
\toprule
\textbf{Configuration} & $\lambda_E$ & \textbf{Final Test Acc} & \textbf{Acc Delta} & \textbf{Hard Sparsity} & \textbf{Pred Energy [mJ]} & \textbf{Energy Fraction} \\
\midrule
Control baseline & $0.00$ & $91.86\%$ & $+0.00\%$ & $6.98\%$ & $1.436\text{ mJ}$ & $104.68\%$ \\
Low penalty & $0.03$ & $91.83\%$ & $-0.03\%$ & $5.70\%$ & $1.226\text{ mJ}$ & $89.35\%$ \\
Moderate penalty & $0.07$ & $91.22\%$ & $-0.64\%$ & $9.43\%$ & $1.146\text{ mJ}$ & $83.49\%$ \\
Substantial pruning & $0.25$ & $91.54\%$ & $-0.32\%$ & $14.58\%$ & $1.040\text{ mJ}$ & $75.79\%$ \\
Strong penalty & $0.60$ & $89.79\%$ & $-2.07\%$ & $18.65\%$ & $0.961\text{ mJ}$ & $70.06\%$ \\
\textbf{Sweet spot target} & $\mathbf{1.20}$ & $\mathbf{89.26\%}$ & $\mathbf{-2.60\%}$ & $\mathbf{21.67\%}$ & $\mathbf{0.898\text{ mJ}}$ & $\mathbf{65.42\%}$ \\
Aggressive pruning & $2.50$ & $85.37\%$ & $-6.49\%$ & $35.08\%$ & $0.670\text{ mJ}$ & $48.85\%$ \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figs/joulenas/pareto_frontier_orin_bs32.pdf}
    \caption{Empirical accuracy--energy Pareto frontier (left) and channel sparsity vs. energy fraction (right) for ResNet-18 on CIFAR-10 under the NVIDIA Jetson AGX Orin batch size 32 operational regime. Pruning pressure successfully drives compute savings when $\lambda_E$ is scaled in accordance with the $13.5\times$ energy amortization.}
    \label{fig:pareto_frontier_bs32}
\end{figure}

\subsubsection{Key empirical findings}

The batch-32 results confirm several critical properties of hardware-aware differentiable pruning:
\begin{enumerate}
    \item \textbf{Confirmation of the scaling prediction}: As predicted by the $13.5\times$ energy drop, penalty values below $0.10$ exert minimal pruning force ($<10\%$ sparsity). Significant channel pruning begins precisely at $\lambda_E \ge 0.25$ (which corresponds to $\lambda_E \approx 0.018$ in the batch-1 regime).
    \item \textbf{Free pruning with zero accuracy penalty ($\lambda_E = 0.25$)}: At $\lambda_E = 0.25$, the network prunes $14.58\%$ of its channels and reduces energy by \textbf{$24.21\%$} ($1.040\text{ mJ}$ vs. $1.372\text{ mJ}$) while maintaining a test accuracy of $91.54\%$, virtually indistinguishable from the unpruned baseline ($91.86\%$).
    \item \textbf{The operational sweet spot ($\lambda_E = 1.20$)}: Pushing the penalty to $\lambda_E = 1.20$ delivers a robust \textbf{$34.58\%$ energy reduction} ($0.898\text{ mJ}$) with $21.67\%$ structural sparsity and an accuracy of $89.26\%$. This operating point provides the highest energy saving margin while comfortably preserving the $>89\%$ accuracy threshold.
    \item \textbf{Aggressive compression ($\lambda_E = 2.50$)}: At $\lambda_E = 2.50$, the model sheds more than a third of its filters ($35.08\%$ sparsity) and cuts inference energy by more than half (\textbf{$51.15\%$ energy reduction} down to $0.670\text{ mJ}$), with accuracy settling at $85.37\%$.
\end{enumerate}
These experiments show that when batching amortizes GPU launch overheads, JouleNAS prunes channels in direct proportion to arithmetic workload, demonstrating that our hardware-aware search generalizes across both execution batch sizes and compute regimes.
