...
To establish a single source of truth, JouleQuest was updated with a canonical, \textbf{spec-driven architecture pipeline} (\texttt{layers/pruned\_resnet.py}):
\begin{enumerate}
    ...
    \item \textbf{Backwards compatibility}: The tool \texttt{materialize\_pruned\_model.py} is retained strictly for offline inspection and verification (the former \texttt{--update-resnet} option was deprecated and removed), and historical named model classes are preserved as compatibility aliases.
\end{enumerate}
...
To close the loop on the embedded GPU target and explore batch scaling behavior on physical silicon, we deployed the materialized \texttt{PrunedOrinBs32ResNet18} (derived from the optimal $\lambda_E = 1.20$ checkpoint) alongside the dense baseline on the physical NVIDIA Jetson AGX Orin at batch size 32. While earlier testing used a temporary script (\texttt{validate\_agx\_orin\_bs32.sh}), that dedicated wrapper was deprecated and removed in favor of the general campaign launcher:
\begin{lstlisting}
BACKEND=cuda NETWORK_BATCH_SIZE=32 \
./run_measurement_campaign.sh \
  --board agx_orin_bs32 \
