...
This chapter introduces \textbf{JouleQuest}, an automated hardware measurement pipeline. JouleQuest executes neural network workloads (ranging from isolated \texttt{Conv2d} layers to complete ResNet-18 models) on target edge boards and profiles their power consumption using an INA226 power sensor. Once hardware and network settings are configured, a single command orchestrates the entire measurement campaign.

The repository's official documentation provides technical references: \texttt{README.md} outlines primary commands, and the \texttt{docs\_new/} directory provides detailed architectural guides:
\begin{itemize}
    \item \texttt{experiment\_campaign.md} --- the complete measurement matrix: layer families, channel dimensions, boards, and environmental constraints.
    \item \texttt{adaptive\_burst\_measurement.md} --- the dynamic inferences-per-burst protocol, explaining how adaptive sizing maintains measurement stability within target acquisition windows (Section~\ref{subsec:adaptive_burst}).
    \item \texttt{joulegrad\_energy\_estimation.md} --- the pipeline transforming raw sensor traces into structured lookup tables consumed by JouleGrad.
    \item \texttt{cli\_acquisition\_report.md} --- instructions for executing headless measurements from the Linux terminal rather than a GUI, verifying serial baud rates and sensor registers.
    \item \texttt{energy\_lookup\_table\_measurement\_audit.md} --- statistical validation verifying that compiled lookup tables faithfully reflect empirical board measurements.
    \item \texttt{run\_manager\_}\allowbreak\texttt{update.md} --- architectural overview of new features and execution safeguards added to \texttt{run\_manager.py}.
    \item \texttt{terminal\_acquisition\_full\_report.md} --- detailed internal documentation on the headless acquisition daemon, prerequisites, and automated multi-burst sequencing.
    \item \texttt{pruned\_resnet\_}\allowbreak\texttt{measurement.md} --- documents the spec-driven workflow for pruned ResNet-18 composition measurements, detailing how a single JouleNAS pruning log drives dynamic model construction, component layer scheduling, deterministic architecture hashing, and physical validation.
\end{itemize}
This chapter summarizes the end-to-end measurement workflow, highlighting how acquisition protocols and post-processing tools operate together.

\section{Hardware configuration}
Depending on the board's electrical requirements, the test bench supports two power configurations:
\begin{itemize}
    \item \textbf{Low power} (Figure \ref{fig:low_supplier}): Raspberry~Pi~5, Jetson~Nano. The power supply provides integrated terminal connectors ready for board headers without custom cabling.
    \item \textbf{High power} (Figure \ref{fig:high_supplier}): Jetson Orin~Nano, Jetson AGX~Orin. The supply interfaces via dedicated barrel cable extensions (Figure \ref{fig:high_extend}).
\end{itemize}
...
\begin{itemize}
    \item \textbf{The PC}: Connected to the TI-SCB via USB, it executes the orchestrator (\path{automated_measurement.py}) and the serial logger (\path{ina226_serial_logger.py}), streaming raw INA226 samples to CSV. Requirements: Python and \texttt{pyserial}.
    \item \textbf{The board} (Jetson, Raspberry~Pi~5, etc.): Runs the inference workloads. It requires \path{run_manager.py}, \path{runner.py}, \path{base_runner.py}, and the \texttt{layers/} directory for full networks and component suites.
\end{itemize}

To provision a \emph{new} board, transfer the inference runner scripts over SSH:
...
On the PC controller, install the acquisition dependencies (\texttt{pyserial} for serial logging; \texttt{pandas}, \texttt{scikit-image}, and \texttt{matplotlib} for report generation):
...
SSH authentication is the most common operational failure point: \textbf{if SSH prompts for a password, the run fails immediately}, because the automated controller runs with \texttt{BatchMode=yes}. Configure key-based authentication \emph{before} launching campaigns---typically with \texttt{ssh-copy-id}---and remember to authorize the jump host if connecting through a gateway.

The final prerequisite is configuring the target connection in \path{measurement_hosts.local.json}:
...
While you can run JouleQuest without reading this section, understanding its internals helps you interpret the resulting traces and diagnose abnormal runs; it also explains key parameter choices like sampling rates and idle windows. Everything described below happens automatically.

\subsection{One experiment, start to finish}

Each experiment is driven by \texttt{automated\_measurement.py} on the PC, and executes the same sequence:

\begin{enumerate}
    \item The PC starts the experiment on the board over SSH.
    \item The board executes a \textbf{warm-up} burst (not measured): because initial forward passes incur framework and CUDA context initialization latency, this initial burst is discarded.
    \item The board runs a short \textbf{calibration} burst (not measured): it profiles a few small batches to determine single-inference latency, and computes how many inferences are needed to fill the target measurement window.
    \item The board signals the PC to \textbf{start recording}: the INA226 logger begins streaming power samples to a CSV file.
    \item The \textbf{measured cycles} execute: multiple workload bursts interleaved with idle pauses, preceded and followed by baseline windows so static platform power can be subtracted during analysis.
    \item The board signals the PC to \textbf{stop recording}. The logger closes the CSV file \emph{before} the board generates its summary manifest, preventing disk I/O from contaminating the power trace.
    \item The PC retrieves the manifest JSON, stores it alongside the CSV, and removes the remote copy on the board (unless \texttt{--keep-remote-manifest} is passed for debugging).
\end{enumerate}
...
The manifest summarizes experiment execution: layer shape, burst duration, measured sampling rate, and a \texttt{quality\_status} flag. The process exit code indicates status: \texttt{0} (success), \texttt{1} (execution failure), \texttt{2} (model executed but recording is invalid), or \texttt{130} (interrupted cleanly with \texttt{Ctrl+C}).
...
Older versions of this project ran the same huge number of inferences for every layer. That worked, but it wasted a lot of time on slow layers while being too short to be meaningful for fast ones. Now the length of each burst is computed automatically, so that every measured region is long enough both in \emph{time} and in \emph{number of samples}:
...
\end{itemize}
In words: the burst must last at least \texttt{--target\_burst\_seconds}, \emph{and} it must contain enough samples for the statistics to be meaningful---whichever condition requires more time sets the duration, scaled by the safety margin. In our benchmarking campaigns, we configured a minimum of $100$ active samples (\texttt{--min\_active\_samples 100}) rather than a fixed time window, ensuring consistent statistical power across both fast and slow layers.
...
(\texttt{Models/CUDA/}\allowbreak\texttt{Linear/}\allowbreak\texttt{Linear\_64\_64.pt}) is enough --- if the file
...
\begin{itemize}
    ...
    with \texttt{--experiment-}\allowbreak\texttt{cooldown-seconds 60}.
\end{itemize}
...
The full execution parameters for each campaign are orchestrated by \texttt{run\_}\allowbreak\texttt{measurement\_}\allowbreak\texttt{campaign.sh} and enforced on the target board by \texttt{run\_manager.py}. To guarantee that measurements across different boards (Raspberry~Pi~5, Jetson Nano, Jetson Orin Nano, Jetson AGX Orin) are reproducible, thermally stable, and statistically reliable, the campaign enforces a standardized set of operational hyperparameters.
...
\begin{itemize}
    ...
    \item \textbf{pruned\_resnet18} (dynamic count, e.g. 23 experiments): a spec-driven validation suite that schedules the dense baseline, the runtime-configured pruned ResNet-18 instantiated from a JouleNAS pruning JSON log, and all its constituent \texttt{Conv2d} and \texttt{Linear} operations for physical composability verification (detailed in Chapter~\ref{sec:joulenas_to_joulequest}).
\end{itemize}
...
By default, the script reads all colocated CSV and JSON manifest pairs in \texttt{--data-dir}, generates per-measurement plot PDFs in \texttt{--plot-dir} (defaulting to \texttt{measurements/}\allowbreak\texttt{Plot/}\allowbreak\texttt{<data\_dir\_name>}), and writes the consolidated summary table directly into \texttt{measurements/}\allowbreak\texttt{lookup\_summaries/}\allowbreak\texttt{summaries/}\allowbreak\texttt{<BOARD\_LABEL>.csv}. The summary destination is controlled by \texttt{--summary-dir} (defaulting to \texttt{measurements/}\allowbreak\texttt{lookup\_summaries/}\allowbreak\texttt{summaries}) and \texttt{--output-name} (aliased as \texttt{--summary-name}, defaulting to \texttt{<data\_dir\_name>.csv}).
...
inferences (\texttt{--min-inferences-}\allowbreak\texttt{for-trimming}, 10 by default), where
...
Our working \textbf{hypothesis} attributes this discrepancy primarily to a \textbf{cache effect}. Measured in isolation, each operation executes in a tight loop over its own tensors: weights and activations stay hot in the cache hierarchy, and measured energy reflects almost pure arithmetic computation. Inside a complete network, each layer consumes the output of the preceding one, expanding the working set beyond on-chip cache capacity; data continuously traverse cache levels and DRAM, incurring memory bus overhead that standalone layer measurements never see. The Raspberry~Pi~5, with the smallest caches among the evaluated boards, exhibits the largest gap; the AGX~Orin, with a substantially larger cache hierarchy, can instead exploit producer--consumer locality between consecutive layers, shifting composition favorably. While the sign and magnitude of the observed gaps support this hypothesis, verifying it directly with hardware performance counters (PMUs) remains an important future objective.

Keep this gap in mind when evaluating JouleGrad's estimates: because they are compiled from standalone measurements, on cache-constrained boards they can underestimate the energy of a large composed network by $20$--$30\%$, as demonstrated by the Raspberry~Pi~5 baseline.
...
\begin{itemize}
    ...
    \item \texttt{docs\_new/pruned\_resnet\_measurement.md} --- the spec-driven
    pipeline and physical validation protocol.
\end{itemize}
